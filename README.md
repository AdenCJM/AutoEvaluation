...
Open the project in Claude Code — either open the folder in VS Code with the Claude Code extension, or run `claude` in the project directory. Then either type `/autoeval` or just tell Claude what you want:
...
`/autoeval` is a slash command skill bundled with the project. Running `./start.sh` installs it automatically — after that it's always available in Claude Code.
...
├── .claude/
│   ├── settings.json         # Auto-approve rules for Claude Code (generated by setup.py, gitignored)
│   └── skills/autoeval/      # /autoeval slash command (auto-installed by start.sh)
├── prompts/
